...
#else
  #pragma message ("Für Platine Version 2")
...
void setup() {
  ...
  */
  ...
  if (!GetJSON()) {  // RKI holen
    ...
    Serial.printf("Für AGS %s (%s) -> Inzidenz: %f (int: %d)\n", Daten.InzAGS, County, Inzidenz, InzidenzInt);
  }
  NextRKI = millis() + RKI_INTERVAL;
}
...
void loop() {
  ...
  if (now >= (LastPing + TIME_TO_PING)) {
    ...
    /*if (!Ping.ping(WiFi.gatewayIP(), 1)) {
      // Nicht im Netz
      strcpy(Daten.CrashLog, "Ping-Fehler");
      PutEE();
...
void DisplayData(void) {
  ...
  if (Daten.ShowMore) {
    switch (tm.tm_sec) {
      ...
      case 4:
        if (Daten.ShowInz)
          if (InzidenzInt < 10000)
            SAA.print(InzidenzInt, 2);
          else
            SAA.print(InzidenzInt / 10, 1);
        else
          MakeTimeDisplay();
        break;
      case 30:
